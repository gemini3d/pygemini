...
def conductivity(
    ...
    m_i = xr.DataArray(dims=("species"), 
                       coords=[("species", ["O+", "NO+", "N2+", "O2+", "N+", "H+"])], 
                       data=np.array([16., 30., 28., 32., 14., 1.])*const.m_p)
    ...
    sigma_P = (sigma_i * nu_i**2 / (nu_i**2 + Omg_i**2)).sum(dim=("species")) + sigma_e * nu_e**2 / (nu_e**2 + Omg_e**2)

    
    # Hall conductivity (S&N equation 5.120)
    sigma_H = -(sigma_i * nu_i*Omg_i / (nu_i**2 + Omg_i**2)).sum(dim=("species")) + sigma_e * nu_e*Omg_e / (nu_e**2 + Omg_e**2)
