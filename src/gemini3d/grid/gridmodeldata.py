...
def model2magcoords(xg,parm,lalt,llon,llat,altlims=None,mlonlims=None,mlatlims=None):    
    ...
    inds1=range(2,lx1+2); inds2=range(2,lx2+2); inds3=range(2,lx3+3);
    x1=xg["x1"][inds1]; x2=xg["x2"][inds2]; x3=xg["x3"][inds3];
    
    # set some defaults if not provided by user
    if altlims==None:
        altlims=np.array( [min(alt.flatten())+0.0001,max(alt.flatten())-0.0001] )
        mlonlims=np.array( [min(mlon.flatten())+0.0001,max(mlon.flatten())-0.0001] )
        mlatlims=np.array( [min(mlat.flatten())+0.0001,max(mlat.flatten())-0.0001] )
    ...
    [MLONi,ALTi,MLATi]=np.meshgrid( (mloni,alti,mlati) )
    ...
    [X2,X1,X3]=np.meshgrid(x2,x1,x3)
    parmi=scipy.interpolate.interpn( (X2,X1,X3) ,parm, (x2i,x1i,x3i))
